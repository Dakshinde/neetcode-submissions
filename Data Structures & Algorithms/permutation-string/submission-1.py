...
"""
    The secret to staying at O(n) time is ensuring that everything inside your for j in range(len(s2)) loop executes in O(1) constant time.

    Changing a dictionary value? O(1).

    Deleting a key? O(1).

    Comparing two dictionaries with a maximum of 26 keys? O(1).

Since the work inside the loop is O(1), your total time is strictly bound to how many times the loop runs: n×O(1)=O(n).
 """
